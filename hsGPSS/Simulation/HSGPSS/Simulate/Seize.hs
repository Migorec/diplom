...
release' ss (SBlock (Release f) ix) transact = 
    case dc $ facilities ss ! f of
        [] -> ss{facilities = defaultUpdate (F.release (currentTime ss)) f $ facilities ss,
                 cec = transact{currentBlock = ix, nextBlock = ix + 1} : cec ss}
        t:ts -> ss{facilities = defaultUpdate (\f -> (F.capture (currentTime ss) $ F.release (currentTime ss) f){dc = ts}) f $ facilities ss,
                   cec = transact{currentBlock = ix, nextBlock = ix + 1} : t{nextBlock = nextBlock t + 1, currentBlock = nextBlock t, state = Active} : cec ss}
